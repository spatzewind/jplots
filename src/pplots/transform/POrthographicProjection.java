...
public class POrthographicProjection implements PProjection {
	
	private double cenlat,cenlon, updir;
	private double[][] rotmat_p2x, rotmat_x2p;
	
	public POrthographicProjection(double center_longitude, double center_latitude, double upward_direction, boolean in_degree) {
		double fac = in_degree ? PPlotMath.DEG_TO_RAD : 1d;
	...
	public double[] fromPROJtoLATLON(double x, double y, boolean output_in_degree) {
		...
		double fac = output_in_degree ? PPlotMath.RAD_TO_DEG : 1d;
	...
	public double[] fromLATLONtoPROJ(double u, double v, boolean input_in_degree) {
		if(!(Double.isFinite(u) && Double.isFinite(v)))
			return new double[] {Double.NaN, Double.NaN};
		double fac = input_in_degree ? PPlotMath.DEG_TO_RAD : 1d;
	...
	public void drawBorder(PAxis ax, PGroupShape s) {
		...
		float x1 = (float) (p[0]+p[2]*Math.max(0d, Math.min(1d, PPlotMath.dlerp(-EARTH_RADIUS_MEAN,r[0],r[1],0d,1d))));
		float y1 = (float) (p[1]+p[3]*Math.max(0d, Math.min(1d, PPlotMath.dlerp(                0d,r[2],r[3],0d,1d))));
		for(int i=-35; i<=36; i++) {
			double bx = EARTH_RADIUS_MEAN*Math.cos(5*i*PPlotMath.DEG_TO_RAD);
			double by = EARTH_RADIUS_MEAN*Math.sin(5*i*PPlotMath.DEG_TO_RAD);
			float x2 = (float) (p[0]+p[2]*Math.max(0d, Math.min(1d, PPlotMath.dlerp(bx,r[0],r[1],0d,1d))));
			float y2 = (float) (p[1]+p[3]*Math.max(0d, Math.min(1d, PPlotMath.dlerp(by,r[2],r[3],1d,0d))));
			s.addChild(new PLineShape(x1, y1, x2, y2));
			x1 = x2;
			y1 = y2;
		}
	}
