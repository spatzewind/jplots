...
public class PEquirectangularProjection implements PProjection {
	
	private double cenlat_d,cenlon_d;
	private double cenlat_r,cenlon_r;
	private double radaequ,radpol;
	
	public PEquirectangularProjection(double center_latitude, double center_longitude, boolean in_degree) {
		double fac = in_degree ? PPlotMath.DEG_TO_RAD : 1d;
		cenlat_r = center_latitude * fac;
		cenlon_r = center_longitude * fac;
		fac = in_degree ? 1d : PPlotMath.RAD_TO_DEG;
