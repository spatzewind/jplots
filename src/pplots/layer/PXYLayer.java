		...
		for(int i=0; i+1<xarrayx.length; i++)
			if(Double.isFinite(xarrayx[i]) && Double.isFinite(xarrayx[i+1]) &&
				...
				while(lpos<l) {
					...
					if(xf1<p[0]      && xf2>=p[0])      { yf1 = PPlotMath.flerp(p[0],      xf1, xf2, yf1, yf2); }
					if(xf1>p[0]+p[2] && xf2<=p[0]+p[2]) { yf1 = PPlotMath.flerp(p[0]+p[2], xf1, xf2, yf1, yf2); }
					if(xf2<p[0]      && xf1>=p[0])      { yf2 = PPlotMath.flerp(p[0],      xf1, xf2, yf1, yf2); }
					if(xf2>p[0]+p[2] && xf1<=p[0]+p[2]) { yf2 = PPlotMath.flerp(p[0]+p[2], xf1, xf2, yf1, yf2); }
					
					if(yf1<p[1]      && yf2>=p[1])      { xf1 = PPlotMath.flerp(p[1],      yf1, yf2, xf1, xf2); }
					if(yf1>p[1]+p[3] && yf2<=p[1]+p[3]) { xf1 = PPlotMath.flerp(p[1]+p[3], yf1, yf2, xf1, xf2); }
					if(yf2<p[1]      && yf1>=p[1])      { xf2 = PPlotMath.flerp(p[1],      yf1, yf2, xf1, xf2); }
					if(yf2>p[1]+p[3] && yf1<=p[1]+p[3]) { xf2 = PPlotMath.flerp(p[1]+p[3], yf1, yf2, xf1, xf2); }
					if(xf1>=p[0] && xf1<=p[0]+p[2] && xf2>=p[0] && xf2<=p[0]+p[2] &&
							yf1>=p[1] && yf1<=p[1]+p[3] && yf2>=p[1] && yf2<=p[1]+p[3]) {
						if(li%2==0 && ldif>0d)
							xyShape.addChild(new PLineShape(xf1,yf1,xf2,yf2));
						loff += ldif;
						switch(li) {
