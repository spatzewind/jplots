...
public class JRectShape extends JPlotShape {
	...
	public JRectShape(float x1, float y1, float x2, float y2, float corner, int inner_colour, int outer_colour, float stroke_weight, boolean filled) {
		xtl = Math.min(x1, x2);
		ytl = Math.min(y1, y2);
		xbr = Math.max(x1, x2);
		ybr = Math.max(y1, y2);
	...
	public void draw(JPlot plot, PGraphics g) {
		...
		switch(g.rectMode) {
			case PApplet.CENTER:
				if(cr>0f) g.rect(0.5f*(xtl+xbr), 0.5f*(ytl+ybr), xbr-xtl, ybr-ytl, cr);
				else      g.rect(0.5f*(xtl+xbr), 0.5f*(ytl+ybr), xbr-xtl, ybr-ytl); break;
			case PApplet.CORNER:
				if(cr>0f) g.rect(xtl, ytl, xbr-xtl, ybr-ytl, cr);
				else      g.rect(xtl, ytl, xbr-xtl, ybr-ytl); break;
			case PApplet.CORNERS:
				if(cr>0f) g.rect(xtl, ytl, xbr, ybr, cr);
				else      g.rect(xtl, ytl, xbr, ybr); break;
			default: break;
		}
	}
}
