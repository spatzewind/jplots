...
public class GeometryTools {
	...
	public static void checkCutC(List<JDPoint> p, List<double[]> c, JDPoint center, double radius, double[] minus_x_dir) {
		if(p.size()<2) return;
		int k = p.size()-2;
		int l = p.size()-1;
		...
		if( Math.abs(uk-crit)<eps && Math.abs(ul-crit)<eps ) {
			...
			if(minus_x_dir[0]==0d && minus_x_dir[1]==0d) {
				...
				double mxdR = 1d / Math.sqrt(
						minus_x_dir[0]*minus_x_dir[0] + minus_x_dir[1]*minus_x_dir[1] );
				minus_x_dir[0] *= mxdR;
				minus_x_dir[1] *= mxdR;
			} else {
...
//								" v="+vs+"|"+ve+" f="+maxf);
	...
	private static void insert_circlepoints(List<JDPoint> sub_polygon, JDPoint center, double radius, double angle_resolution, double[] x_dir) {
	...
	private static void add_if_negative(List<JDPoint> points, List<Double> ids, double[] norm, double value, double epsilon) {
	...
	public static List<JDPoint[]> SutherlandHodgmanAlgorithm(JDPoint[] points, double[] normal, double crit, double eps, List<JDPoint> inserts) {
...
//	}
	...
	public static List<JDPoint[]> SutherlandHodgmanAlgorithmC(JDPoint[] points, double radius, JDPoint center, double angle_resolution_degree) {
		List<JDPoint[]> res = new ArrayList<>();
		if(points==null) return res;
		...
		for(int c=cutids.size()-1; c>=0; c--) {
			...
			boolean overlaps = false;
			for(double[] cj: cutids) {
				if(Math.abs(cj[0]-ci[0])<1.0e-10d) continue;
