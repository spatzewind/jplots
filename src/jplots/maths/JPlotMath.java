...
public class JPlotMath {
	...
	public static float[] lerp(float valA, float valB, float[] fraction) {
		...
		for(int i=0; i<fraction.length; i++) res[i] = valA + fraction[i]*(valB-valA);
		return res; }
	...
	public static float nlerp(float valA, float valB, float fraction) {
		if(Float.isNaN(valA) && fraction>=0.4999f) return valB;
		if(Float.isNaN(valB) && fraction<=0.5001f) return valA;
		return valA + fraction*(valB-valA); }
	public static float[] nlerp(float valA, float valB, float[] fraction) {
		float[] res = new float[fraction.length];
		for(int i=0; i<fraction.length; i++)
			res[i] = Float.isNaN(valA) && fraction[i]>=0.4999f ? valB :
					 Float.isNaN(valB) && fraction[i]<=0.5001f ? valA :
					 valA + fraction[i]*(valB-valA);
		return res; }
	public static double nlerp(double valA, double valB, double fraction) {
		if(Double.isNaN(valA) && fraction>=0.4999d) return valB;
		if(Double.isNaN(valB) && fraction<=0.5001d) return valA;
		return valA + fraction*(valB-valA); }
	public static double[] nlerp(double valA, double valB, double[] fraction) {
		double[] res = new double[fraction.length];
		for(int i=0; i<fraction.length; i++)
			res[i] = Double.isNaN(valA) && fraction[i]>=0.4999d ? valB :
					 Double.isNaN(valB) && fraction[i]<=0.5001d ? valA :
					 valA + fraction[i]*(valB-valA);
		return res; }
	...
	public static double[] optimalLogarithmicTicks(double vmin, double vmax, int maxTickCount) {
	    ...
	    for(int pf: factors) {
	        ...
	        switch(pf) {
	            case  2: effTickCount *= (1d-Math.log10(5.0d))/0.50d; break;
	            case  5: effTickCount *= (1d-Math.log10(8.0d))/0.20d; break;
	            case  9: effTickCount *= (1d-Math.log10(9.0d))/0.10d; break;
	            case 19: effTickCount *= (1d-Math.log10(9.5d))/0.05d; break;
	            case 49: effTickCount *= (1d-Math.log10(9.8d))/0.02d; break;
	            case 99: effTickCount *= (1d-Math.log10(9.9d))/0.01d; break;
	            default: break;
	        }
	        preFac = pf;
	        //System.out.println("preFac="+preFac+"    jin/jax="+iin+"/"+iax+"  effTickCount="+effTickCount);
	        if(iax-iin<effTickCount) break;
	    }
	    if(preFac>0) {
	        ...
	        for(int j=0; j<=idiff; j++) {
	            ...
	            if((iin+j)%preFac!=0)
	                switch(preFac) {
	                    case  2: ticks[j+2] *= 5d; break;
	                    case  5: ticks[j+2] *= 2d*((iin+j)%5); break;
	                    case  9: ticks[j+2] *= 1+(iin+j)%9; break;
	                    case 19: ticks[j+2] *= 1d + 0.5d*((iin+j)%19); break;
	                    case 49: ticks[j+2] *= 1d + 0.2d*((iin+j)%49); break;
	                    case 99: ticks[j+2] *= 1d + 0.1d*((iin+j)%99); break;
	                    default: break;
	                }
	        }
	        ticks[0] = -1d;
	...
	public static class DateTime {
		...
		public DateTime() {
			...
			days = 2440587L+d; dayfraction = (double) s / 86400000d; }
		public DateTime(long i, double f) {
			days = i; dayfraction = f; }
		public DateTime(String value, String _cal) {
			String cal = _cal==null ? "proleptic" : _cal;
			DateTime refDate = null;
			if(refDate==null && cal.equalsIgnoreCase("gregorian")) refDate = parseGregorian(value.trim());
			if(refDate==null && cal.equalsIgnoreCase("julian"))    refDate = parseJulian(value.trim());
			if(refDate==null && cal.equalsIgnoreCase("proleptic")) refDate = parseProleptic(value.trim());
			if(refDate==null)
		...
		public double getDayFraction() { return dayfraction; }
		...
		public static DateTime fromDouble(double _in, String _unit, String _cal) {
			...
			if(uuu.length<1 || uuu.length>2)
				throw new IllegalArgumentException("Illegal unit-string. Expected format: <sec,min,days,...> since <reference dd.mm.yyyy hh:mm:ss>");
			...
			if(refDate==null && cal.equalsIgnoreCase("julian"))    refDate = parseJulian(uuu[1].trim());
			if(refDate==null && cal.equalsIgnoreCase("proleptic")) refDate = parseProleptic(uuu[1].trim());
			if(refDate==null)
				throw new IllegalArgumentException("Unknown calendar '"+_cal+"'");
		...
		public int[] toDate(String _cal) {
			...
			if(calID==3 || (calID==1 && year>1582)) leap = (year%400==0 ? true : year%100==0 ? false : year%4==0);
		...
		public String toDateString(String _cal, boolean withAbbr) {
			int[] dd = toDate(_cal);
			int cy = PApplet.year(); cy -= cy%10;
			int dy = cy - (cy%100);
			if(dd[2]>=cy-80 && dd[2]<=cy+20 && withAbbr)
				return dd[0]+"."+dd[1]+".'"+(dd[2]-dy)+"  "+PApplet.nf(dd[3],2)+":"+PApplet.nf(dd[4],2)+":"+PApplet.nf(dd[5],2);
			return dd[0]+"."+dd[1]+"."+dd[2]+" "+PApplet.nf(dd[3],2)+":"+PApplet.nf(dd[4],2)+":"+PApplet.nf(dd[5],2);
		}
		public String format(String _format, String _cal) {
			...
			if(res.contains("y")) {
				...
				String ys = "";
				switch(b+1-a) {
					case 1: int y1=dd[2]/10-(dd[2]<0?1:0);      ys = ""+(dd[2]-10*y1); break;
					case 2: int y2=dd[2]/100-(dd[2]<0?1:0);     ys = dd[2]>-99&&dd[2]<0 ? ""+dd[2] : PApplet.nf(dd[2]-100*y2,2); break;
					case 3: int y3=dd[2]/1000-(dd[2]<0?1:0);    ys = dd[2]>-999&&dd[2]<0 ? ""+dd[2] : PApplet.nf(dd[2]-1000*y3,3); break;
					case 4: int y4=dd[2]/10000-(dd[2]<0?1:0);   ys = dd[2]>-9999&&dd[2]<0 ? ""+dd[2] : PApplet.nf(dd[2]-10000*y4,4); break;
					case 5: int y5=dd[2]/100000-(dd[2]<0?1:0);  ys = dd[2]>-99999&&dd[2]<0 ? ""+dd[2] : PApplet.nf(dd[2]-100000*y5,5); break;
					case 6: int y6=dd[2]/1000000-(dd[2]<0?1:0); ys = dd[2]>-999999&&dd[2]<0 ? ""+dd[2] : PApplet.nf(dd[2]-1000000*y6,6); break;
					default: ys = PApplet.nf(dd[2],b+1-a);
				}
		...
		public double toDouble(String _unit, String _cal) {
			...
			String cal = _cal==null ? "proleptic" : _cal;
			DateTime refDate = null;
			if(refDate==null && cal.equalsIgnoreCase("gregorian")) refDate = parseGregorian(uuu[1]);
			if(refDate==null && cal.equalsIgnoreCase("julian"))    refDate = parseJulian(uuu[1]);
			if(refDate==null && cal.equalsIgnoreCase("proleptic")) refDate = parseProleptic(uuu[1]);
			if(refDate==null)
				throw new IllegalArgumentException("Unknown calendar '"+_cal+"'");
		...
		public DateTime add(double diff, String _unit) {
			...
			days        += dti;
			dayfraction += dtf;
			if(dayfraction<0d)  { days--; dayfraction += 1d; };
			if(dayfraction>=1d) { days++; dayfraction -= 1d; };
			return this;
		}
		...
		private static DateTime parseProleptic(String date) { //proleptic gregorian
			...
			if(leap && ddi[2]%100==0) leap = ddi[2]%400==0;
			int dc = dateToDaycount(ddi, leap);
			int y = ddi[2]<0 ? ddi[2] : ddi[2]-1;
			int yQ = y/400 - (y<0?1:0); y -= 400*yQ;
			int yC = y/100 - (y<0?1:0); y -= 100*yC;
			int y4 = y/4 - (y<0?1:0);   y -=   4*y4;
		...
		private static int dateToDaycount(int[] dd, boolean isLeap) {
			switch(dd[1]) {
				case 1:  return dd[0]-1;
				case 2:  return 30+dd[0];
				case 3:  return (isLeap?59:58)+dd[0];
				case 4:  return (isLeap?90:89)+dd[0];
				case 5:  return (isLeap?120:119)+dd[0];
				case 6:  return (isLeap?151:150)+dd[0];
				case 7:  return (isLeap?181:180)+dd[0];
				case 8:  return (isLeap?212:211)+dd[0];
				case 9:  return (isLeap?243:242)+dd[0];
				case 10: return (isLeap?273:272)+dd[0];
				case 11: return (isLeap?304:303)+dd[0];
				case 12: return (isLeap?334:333)+dd[0];
				default: return 0;
			}
		}
		private static int[] daycountToDate(int dc, boolean isLeap) {
			int d = dc+1;
			if(d<32) return new int[] {d, 1}; // january
			d -= 31;
			if(d<29) return new int[] {d, 2}; // february
			if(isLeap && d<30) return new int[] {d, 2};
			d -= (isLeap ? 29 : 28);
			if(d<32) return new int[] {d, 3}; // march
			d -= 31;
			if(d<31) return new int[] {d, 4}; // april
			d -= 30;
			if(d<32) return new int[] {d, 5}; // mai
			d -= 31;
			if(d<31) return new int[] {d, 6}; // june
			d -= 30;
			if(d<32) return new int[] {d, 7}; // july
			d -= 31;
			if(d<32) return new int[] {d, 8}; // august
			d -= 31;
			if(d<31) return new int[] {d, 9}; // september
			d -= 30;
			if(d<32) return new int[] {d,10}; // october
			d -= 31;
			if(d<31) return new int[] {d,11}; // november
			d -= 30;
			if(d<32) return new int[] {d,12}; // december
			throw new RuntimeException("Something went wrong during date-conversion <daycountToDate(dc="+dc+", isLeap="+isLeap+")>");
		}
	}
}
